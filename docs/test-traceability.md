...
| Lifecycle rules | `packages/shared/src/lifecycle.test.ts` | `lifecycle-service.integration.test.ts` | Restart, retry, rollback, audit, assignment/handoff, deletion, role, public-sync state, and stream reconciliation | `lifecycle-transition-handler.postgres.test.ts` | Alpha shell sends authenticated durable commands without browser actors | Direct PDS CRUD is proven; normalized stream status/delete reconciliation is locally integrated | Real two-account browser execution through the live stream |
