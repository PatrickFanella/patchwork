...
| Blocks and reports | Service and typed client validation | Older chat safety fixtures are unavailable in production | Repository retention/deletion, bilateral discovery filtering, and idempotency tests | Authenticated session-derived report/block actors persist through real HTTP; expired optional sessions fail closed | Rendered report details, confirmation, CSRF, success/error behavior | A helper reported and blocked the live requester; cleanup retained one redacted safety receipt | Chat enforcement remains deferred with chat itself; formal policy approval remains |
